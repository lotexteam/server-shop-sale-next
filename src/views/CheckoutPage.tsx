        ...
        onSubmit={async (e) => {
          ...
          try {
              ...
              methodMeta?.requiresAddress
                ? selectedAddress
                  ? {
                      ...
                      postal_code: selectedAddress.zip ?? undefined,
            ...
            const previewPayload = {
              ...
              legal_entity_id: selectedLegal?.id,
              delivery_meta: {
                ...(deliveryMeta || {}),
            ...
            try {
              ...
              setServerPreviewError(
                previewErr instanceof StorefrontApiError
                  ? previewErr.message
                  : previewErr instanceof Error
                    ? previewErr.message
                    : "Сервер отклонил параметры заказа",
              );
              throw previewErr;
            }
            ...
            // Файл реквизитов для счёта — отдельным документом к заказу.
            if (requisitesFile && order?.id) {
              try {
                const fdReq = new FormData();
                fdReq.append("file", requisitesFile);
                fdReq.append("title", "Реквизиты");
                fdReq.append("type", "requisites");
                await uploadOrderRequisites(order.id, fdReq);
              } catch {
                /* заказ уже создан — файл можно прикрепить позже */
              }
            }
      ...
      >
        ...
        <aside className="min-w-0">
          <div className="sticky-below-header surface-card p-6">
                ...
                <li key={l.lineKey ?? l.product.id} className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-caption text-muted-foreground">
                      {isLineOnRequest(l)
                        ? `x ${l.qty} · под заказ`
                        : `x ${l.qty} · ${l.product.price == null ? "под заказ" : formatPrice(l.product.price)}`}
                    </p>
                    {l.warranty?.isPaid && (
                      <p className="text-caption text-muted-foreground">
            ...
            {serverPreviewError && !submitError && (
              <p className="mt-3 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-caption text-destructive">
                {serverPreviewError}
              </p>
            )}
