...
import { fetchAccountOrders, fetchNewsletterStatus, forgotPassword, requestEmailChangeCode, requestPasswordChangeCode, resetPassword, subscribeNewsletter, changeEmail, changeAccountPassword, unsubscribeNewsletter, type AccountOrder } from "@/lib/api";
...
export function AccountPage() {
  ...
  // ── Смена пароля / email с кодом подтверждения на почту ──
  ...
  const [emStage, setEmStage] = useState<"idle" | "code">("idle");
  ...
  const sendEmCode = async () => {
    setEmBusy(true);
    try {
      const message = await requestEmailChangeCode(emNew);
      setEmStage("code");
      push({ variant: "success", title: message });
    } catch (e) {
      push({
  ...
  const submitEmail = async () => {
    setEmBusy(true);
    try {
      const message = await changeEmail(emNew, emCode);
      await updateProfile({ email: emNew });
      setEmStage("idle");
      setEmNew("");
      setEmCode("");
      push({ variant: "success", title: message });
  ...
  if (!isAuthenticated) {
    ...
    const onRegister = async (e: FormEvent) => {      e.preventDefault();
      if (!registerConsent) {
    ...
    const submitForgot = async (e: FormEvent) => {
      ...
      try {
        ...
        push({ variant: "success", title: message });
        setForgotForm({ email: forgotForm.email, code: "", password: "", password2: "" });
            ...
            <div className="space-y-4">
              ...
              {ordersLoading ? (
                ...
                <EmptyState
                  ...
                  description="Оформите заказ на витрине — история появится здесь."
                  action={
            ...
            <div className="surface-card divide-y divide-border p-6">
              ...
              <div className="flex items-center justify-between gap-6 py-4">
                <div className="min-w-0">
                  ...
                  <p className="text-caption text-muted-foreground">
                    Отправляются автоматически на email, указанный в заказе.
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between gap-6 py-4">
              ...
              <div className="flex flex-col gap-3 py-4">
                <div>
                  ...
                  <p className="text-caption text-muted-foreground">
                    {user?.email
                      ? `Код подтверждения придёт на почту ${user.email}.`
              ...
              {/* Смена email с кодом на новый адрес */}
              <div className="flex flex-col gap-3 py-4">
                <div>
                  <span className="text-body-sm font-medium">Email аккаунта</span>
                  <p className="text-caption text-muted-foreground">
                    Текущий: {user?.email || "—"}. Для смены — код подтверждения
                    на новый email.
                  </p>
                </div>
                {emStage === "idle" ? (
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="secondary"
                      disabled={!emNew.includes("@") || emBusy}
                      onClick={() => void sendEmCode()}
                    >
                      Отправить код
                    </Button>
                  </div>
