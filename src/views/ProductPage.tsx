...
import { QuickOrderButton } from "@/components/checkout/QuickOrderButton";
...
export function ProductPage({
  ...
  return <ReadyProductPage product={product} related={related} />;
}
...
function ReadyProductPage({
  ...
  const add = () => {
    ...
    push({
      ...
      description:
        product.price == null
          ? `${product.title} × ${qty} · Под заказ`
          : `${product.title} × ${qty} · ${formatPrice(product.price * qty + extra)}`,
      ...
      <div className="grid gap-5 lg:grid-cols-[auto_1fr] lg:items-start lg:gap-6">
        <div className="flex w-full max-w-[min(100%,26rem)] flex-row items-start gap-2.5">
          ...
          <div className="flex shrink-0 flex-col gap-2">
            {gallery.map((g, i) => (
              ...
              >
                <ProductPhoto src={g} alt="" title={product.title} className="size-full object-cover" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-col justify-between gap-3 lg:min-h-[22.4rem]">
          ...
          <div className="rounded-lg border border-border bg-card p-4 shadow-card sm:p-5">
              ...
              {product.onRequest ? (
                ...
                <span className="min-w-0">
                  <span className="block min-w-0 break-words text-h1 font-bold tabular-nums leading-tight">
                    {product.price == null
                      ? "Под заказ"
                      : formatPrice(product.price + warrantyExtra)}
                  </span>
                  <VatHint />
                </span>
              )}
            ...
            <StockStatus inStock={!product.onRequest} className="mt-1 text-body-sm" />
            ...
            <div className="mt-3 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <Button
                size="lg"
                variant="gradient"
                className="h-auto min-h-12 w-full min-w-0 flex-1 whitespace-normal px-4 text-white"
                onClick={add}
              >
                <span className="inline-flex max-w-full flex-wrap items-center justify-center gap-x-1.5 text-white">
                  <span>В корзину</span>
                  {!product.onRequest && (
              ...
              <QuickOrderButton
                className="h-auto min-h-12 w-full whitespace-normal px-4 sm:w-auto"
                items={[{ product, qty }]}
              />
            </div>
