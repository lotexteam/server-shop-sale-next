...
/**
 * Сборка пользователя сохраняется в localStorage (скользящий TTL из
 * GET /consent/config → storage_ttl_minutes, по умолчанию 60 минут):
...
export function useProductConfigurator(
  ...
  const total = cfg?.basePrice == null ? null : cfg.basePrice + componentsTotal;
