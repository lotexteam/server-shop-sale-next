...
export function buildCompareRows(items: Product[]): CompareRow[] {
  ...
  const main: Omit<CompareRow, "differs">[] = [
    ...
    },
    {
      ...
      label: "Цена",
      values: items.map((p) =>
        p.onRequest || p.price == null ? "Под заказ" : formatPrice(p.price),
      ),
      group: "main",
    },
    {
    ...
    },
    {
      key: "stock",
      label: "Наличие",
      values: items.map((p) => (p.onRequest ? "Под заказ" : "В наличии")),
      group: "main",
    },
  ];
...
export function minPriceIndex(items: Product[]): number {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (items[i].onRequest || items[i].price == null) continue;
    const currentPrice = items[i].price;
    const minimumPrice = min === -1 ? null : items[min].price;
    if (currentPrice != null && (minimumPrice == null || currentPrice < minimumPrice)) {
      min = i;
    }
  }
  return min;
