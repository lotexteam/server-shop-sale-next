...
export function mapApiProduct(p: ApiProduct): Product {
  ...
  const onRequest = p.price?.is_on_request === true || display === null;
  ...
  return {
    ...
    oldPrice:
      display != null && compare != null && compare > display ? Number(compare) : null,
    onRequest,
...
export async function fetchProduct(idOrSlug: string): Promise<Product | null> {  let key = String(idOrSlug || "").trim();
  try {
...
export type CatalogFilterAttr = {
  ...
  max?: number;
  step?: number;
  /** Attribute priority inside its group (same ascending semantics as categories). */
  sort_order?: number;
  group?: string | null;
  /** Group priority; null means the attribute is not assigned to an active group. */
...
function mapSite(raw?: ApiSitePayload | null): ShopSite {
  ...
  return {
    ...
    brandPrimary: String(raw?.brand_primary || "").trim() || "#062531",
    brandAccent: String(raw?.brand_accent || "").trim() || "#C3202F",
...
export type SeoDocument = {
  ...
  /** Волна 4 (SEO-аудит 2026-09): og:site_name / og:image:alt / product:price:* / спеки */
  brand?: string | null;
...
/* ── Newsletter (подписка на новости и акции) ─────────── */
...
/* ── Безопасность аккаунта (смена email/пароля с кодом на почту) ── */

export async function requestEmailChangeCode(newEmail: string): Promise<string> {
  const res = await request<ApiItem<{ message?: string }>>(
    "/auth/email/change-code",
    { method: "POST", auth: true, json: { new_email: newEmail } },
  );
...
export async function changeEmail(
  ...
  const res = await request<ApiItem<{ message?: string }>>("/auth/email", {
    method: "POST",
    auth: true,
    json: { new_email: newEmail, code },
  });
  return res.data?.message || "Email аккаунта изменён.";
}
...
/** Сегмент строки-акцента hero: без href рендерится как обычный span */
export type HomeLinkSegment = {
  label: string;
  href?: string;
};
...
export type HomeContent = {
  ...
  /** Тексты действующей витрины (hero.title, tiles, benefits — см. cms.home сидера sale-server) */
  heroTitle: string | null;
  heroLead: string | null;
  heroNotice: string | null;
  heroReviewsLabel: string | null;
  heroLinks: HomeLinkSegment[];
  heroReviewsLinks: HomeReviewLink[];
  heroRatingWidgetUrl: string | null;
  tiles: HomeTile[];
  benefits: { title: string | null; items: HomeBenefit[] };
...
export async function fetchHomeContent(): Promise<HomeContent> {
  const empty: HomeContent = {
    ...
    heroLead: null,
    heroNotice: null,
    heroReviewsLabel: null,
    heroLinks: [],
    heroReviewsLinks: [],
    heroRatingWidgetUrl: null,
    tiles: [],
  ...
  try {
    ...
    // Пустая строка/пробелы = слот пуст → null (элемент скрывается)
    const str = (v: unknown): string | null => {
      const s = v == null ? "" : String(v).trim();
      return s === "" ? null : s;
    };
    const hero = (d.hero && typeof d.hero === "object" ? d.hero : {}) as Record<
      string,
      unknown
    >;
    const sections = (d.sections && typeof d.sections === "object" ? d.sections : {}) as Record<
      string,
      Record<string, unknown> | undefined
    >;
    const mapStr = (code: string, structured: unknown): string | null =>
      str(textsMap[code]) ?? str(structured);

    const heroLinks = (Array.isArray(hero.links) ? hero.links : [])
      .map((seg): HomeLinkSegment | null => {
        const s = (seg ?? {}) as Record<string, unknown>;
        const label = str(s.label);
        if (!label) return null;
        return { label, href: str(s.href) ?? undefined };
      })
      .filter((seg): seg is HomeLinkSegment => seg !== null);

    const heroReviewsLinks = (Array.isArray(hero.reviews_links) ? hero.reviews_links : [])
      .map((row) => {
    ...
    const mapTile = (n: number): HomeTile | null => {
      const title = str(textsMap[`tiles.${n}.title`]);
      const image = str(textsMap[`tiles.${n}.image`]);
      if (!title || !image) return null;
      return {
        id: `tile-${n}`,
        title,
        text: str(textsMap[`tiles.${n}.text`]) ?? "",
        image,
        href: str(textsMap[`tiles.${n}.href`]) ?? undefined,
        externalHref: str(textsMap[`tiles.${n}.external_href`]) ?? undefined,
        creditPdf: str(textsMap[`tiles.${n}.credit_pdf`]) ?? undefined,
        wide: (str(textsMap[`tiles.${n}.wide`]) ?? "") === "1",
      };
    };
    const tiles = Array.from({ length: MAX_TILES }, (_, i) => mapTile(i + 1)).filter(
      (tile): tile is HomeTile => tile !== null,
    );

    const mapBenefit = (n: number): HomeBenefit | null => {
      const text = str(textsMap[`benefits.${n}.text`]);
      if (!text) return null;
      return { icon: str(textsMap[`benefits.${n}.icon`]) ?? "Check", text };
    };
    const benefitItems = Array.from(
      { length: MAX_BENEFITS },
      (_, i) => mapBenefit(i + 1),
    ).filter((item): item is HomeBenefit => item !== null);
    ...
    return {
      ...
      heroTitle: mapStr("hero.title", hero.title),
      heroLead: mapStr("hero.lead", hero.lead),
      heroNotice: mapStr("hero.notice", hero.notice),
      heroReviewsLabel: mapStr("hero.reviews_label", hero.reviews_label),
      heroLinks,
      heroReviewsLinks,
      heroRatingWidgetUrl: mapStr("hero.rating_widget_url", hero.rating_widget_url),
      tiles,
      benefits: {
        title: mapStr("sections.benefits.title", sections.benefits?.title),
        items: benefitItems,
      },
      discounts: {
        title: mapStr("sections.hot_deals.title", sections.hot_deals?.title),
        empty: mapStr("sections.hot_deals.empty", sections.hot_deals?.empty),
      },
...
// Обложка поста: если не вставлена — не генерируем картинку, просто без обложки.
...
export type CheckoutPayload = {
  ...
  delivery_method_code?: string;
  delivery_category_id?: string | null;
  delivery_category_slug?: string | null;
  ...
  legal_entity_data?: Record<string, unknown> | null;
    ...
    const platforms: ConfiguratorPlatform[] = (data.platforms || [])
      ...
      .map((p) => {
        ...
        return {
          ...
          price:
            p.price_configurator_display == null
              ? null
              : Number(p.price_configurator_display),
          onRequest: p.on_request === true || p.price_configurator_display == null,
          status: p.status ?? null,
          configuratorStatus: p.status_configurator ?? null,
      ...
      const wl = s.whitelist?.length
        ...
        : (s.products || []).map((p) => ({
            ...
            product: {
              ...
              status: p.status,
              status_configurator: p.status_configurator,
            },
          }));
      return {
        id: s.id,
        ...
        options: wl
          ...
          .map((w) => {
            ...
            return {
              ...
              onRequest:
                prod?.on_request === true || prod?.price_configurator_display == null,
              status: prod?.status ?? null,
              configuratorStatus: prod?.status_configurator ?? null,
...
export async function apiSubscribeNewsletter(email: string): Promise<{ message: string }> {  const res = await request<ApiItem<{ status: string; message?: string }>>(
    "/newsletter/subscribe",
    {
      method: "POST",
...
/* ── Account: addresses & legal entities (Bearer + customer) ── */
...
export async function fetchLegalEntities(): Promise<ApiLegalEntity[]> {
  const res = await request<ApiItem<ApiLegalEntity[]>>("/account/legal-entities");
  return res.data || [];
}

export async function apiCreateLegalEntity(
  body: Record<string, unknown>,
): Promise<ApiLegalEntity | null> {
  const res = await request<ApiItem<ApiLegalEntity>>("/account/legal-entities", {
    method: "POST",
    json: body,
  });
...
export async function apiUpdateLegalEntity(
  ...
  await request(`/account/legal-entities/${encodeURIComponent(id)}`, {
    method: "PUT",
    json: body,
  });
}
...
export async function apiUpdateAddress(
  ...
  await request(`/account/addresses/${encodeURIComponent(id)}`, {
    method: "PUT",
    json: body,
  });
}
