...
export function isLineOnRequest(line: CartLine): boolean {
  return line.product.onRequest === true;
}
...
export function cartLineGoodsTotal(line: CartLine): number {
  ...
  return line.product.price == null ? 0 : line.qty * (line.product.price + extra);
}
...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  // пустой — чтение localStorage в useState-инициализаторе даёт hydration
  // mismatch (дерево клиента отличается -> сдвиг useId в шапке).
  ...
  // Первый прогон persist-эффекта — mount с пустым cart ДО гидрации из
  // localStorage: пропускаем, чтобы не затереть сохранённую корзину.
