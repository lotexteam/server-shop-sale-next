...
type FilterCatNode = {
  ...
  slug: string;
  title: string;
  children?: FilterCatNode[];
};

function treeHasChecked(node: FilterCatNode, checked: string[]): boolean {
  if (checked.includes(node.slug)) return true;
  return (node.children ?? []).some((ch) => treeHasChecked(ch, checked));
}
...
function FilterCategoryRow({
  ...
  const kids = (node.children ?? []) as FilterCatNode[];
  ...
  return (
    <div className="space-y-1">
      <div
        ...
        style={depth ? { marginLeft: `${depth * 1.1}rem` } : undefined}
      >
        <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2.5 py-0.5">
          ...
          <span className={cn("flex-1 text-body-sm", depth && "text-muted-foreground")}>{node.title}</span>
        </label>
        {hasKids && (
          <button
            type="button"
      ...
      {hasKids && (
        ...
        >
          ...
          >
            {kids.map((ch) => (
              <FilterCategoryRow key={ch.id} node={ch} depth={depth + 1} checked={checked} onToggle={onToggle} />
            ))}
          </div>
        </div>
      )}
...
export function FilterSidebar({
  ...
  /**
   * Двухуровневая группировка атрибутов: снаружи — категории (когда
   * область каталога — родительская категория, бэкенд помечает блоки
   * category_id/category_name и упорядочивает их «как в категориях»),
   * внутри — группы атрибутов. Порядок категорий — по первому вхождению
   * (порядок ответа); общие блоки (без category_id) — без заголовка.
   */
  const facetGroups = useMemo(() => {
    type AttrGroup = { key: string; name: string; sort: number; facets: CatalogFilterAttr[] };
    type CategoryGroup = { key: string; name: string | null; attrGroups: AttrGroup[] };

    const categoryGroups: CategoryGroup[] = [];
    const categoryByKey = new Map<string, CategoryGroup>();
    const attrGroupByKey = new Map<string, AttrGroup>();

    for (const facet of facets) {
      const categoryKey = facet.category_id ?? "";
      let category = categoryByKey.get(categoryKey);
      if (!category) {
        category = { key: categoryKey, name: facet.category_name ?? null, attrGroups: [] };
        categoryByKey.set(categoryKey, category);
        categoryGroups.push(category);
      }

      const name = facet.group?.trim() ?? "";
      const attrKey = `${categoryKey}::${name || "__ungrouped"}`;
      let attrGroup = attrGroupByKey.get(attrKey);
      if (!attrGroup) {
        attrGroup = {
          key: attrKey,
          name,
          sort: facet.group_sort_order ?? Number.MAX_SAFE_INTEGER,
          facets: [],
        };
        attrGroupByKey.set(attrKey, attrGroup);
        category.attrGroups.push(attrGroup);
      }
      attrGroup.sort = Math.min(attrGroup.sort, facet.group_sort_order ?? Number.MAX_SAFE_INTEGER);
      attrGroup.facets.push(facet);
    }

    for (const category of categoryGroups) {
      category.attrGroups = category.attrGroups
        .map((attrGroup) => ({
          ...attrGroup,
          facets: [...attrGroup.facets].sort((a, b) =>
      ...
      >
        <AccordionItem value="cat">
          ...
          <AccordionContent>
            ...
            />
            <div className="max-h-[min(24rem,50vh)] space-y-2 overflow-y-auto overscroll-contain pr-1">
        ...
        {facetsLoading && facets.length === 0 && (
          <p className="py-3 text-caption text-muted-foreground">Загрузка…</p>
        )}
