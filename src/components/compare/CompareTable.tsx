      ...
      <table className="w-full min-w-[640px] border-collapse text-left">
        <thead>
          <tr className="border-b border-border">
            <th className="sticky left-0 z-20 w-40 bg-card p-4 text-caption font-semibold uppercase tracking-wide text-muted-foreground sm:w-48">
              Параметр
            </th>
            {items.map((p, i) => (
              <th key={p.id} className="min-w-[200px] max-w-[260px] p-4 align-top">
                <div className="relative flex flex-col gap-3">
                  <button
                    type="button"
                    onClick={() => onRemove(p.id)}
                    className="absolute -right-1 -top-1 flex size-8 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-sm transition-colors hover:border-destructive/40 hover:text-destructive"
                  ...
                  <Button size="sm" variant="gradient" className="w-full text-white" onClick={() => onAddToCart(p)}>
