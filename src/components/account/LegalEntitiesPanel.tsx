...
export function LegalEntitiesPanel() {
  ...
  const save = async () => {
    ...
    try {
      ...
      push({
        variant: "error",
        title: "Не удалось сохранить организацию",
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {!e.isDefault && (
                  <Button
                    ...
                    variant="secondary"
                    onClick={() =>
                      void setDefaultLegalEntity(e.id).catch((err) =>
                        push({
                          variant: "error",
                          title: "Не удалось сделать основным",
                          description: err instanceof Error ? err.message : undefined,
                        }),
                      )
                    }
                ...
                <Button
                  ...
                  onClick={() => {
                    void removeLegalEntity(e.id)
                      .then(() => push({ variant: "info", title: "Организация удалена" }))
                      .catch((err) =>
                        push({
                          variant: "error",
