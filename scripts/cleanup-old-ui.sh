#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# ОЧИСТКА СТАРОЙ VITE-ВИТРИНЫ (server-shop-sale-ui): контейнеры, образы, кеш.
# Запускать ПОСЛЕ успешного переключения на server-shop-sale-next
...
#   ./scripts/cleanup-old-ui.sh --old-ui /path/to/sale-ui
#
...
if [ -z "$OLD_UI" ] || [ ! -d "$OLD_UI" ]; then
  echo "✗ Каталог старой витрины не найден. Укажите: --old-ui /path/to/server-shop-sale-ui" >&2
  exit 1
fi
...
[ -n "$OLD_PROJECT" ] || OLD_PROJECT="server-shop-sale-ui"
...
[ -n "$OLD_IMAGE" ] || OLD_IMAGE="ghcr.io/local/server-shop-sale-ui:latest"
