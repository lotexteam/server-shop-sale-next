...
# ─────────────────────────────────────────────────────────────────────────────
# Миграция Vite-витрины (server-shop-sale-ui) → Next.js (server-shop-sale-next).
...
detect_old_ui() {
  ...
  local _cand
  for _cand in "$ROOT/../server-shop-sale-ui" "$ROOT/../sale-ui" "$ROOT/../server-shop-sale-ui"; do
...
do_migrate() {
  ...
  if ! _old="$(detect_old_ui)"; then
    ...
    echo "  Укажите: ./scripts/update.sh --migrate --old-ui /path/to/server-shop-sale-ui" >&2
    return 1
  fi
  OLD_UI="$(cd "$_old" && pwd)"
  ...
  local OLD_PROJECT="${COMPOSE_PROJECT_NAME:-server-shop-sale-ui}"
  ...
  if [ ! -f "$ROOT/.env.prod" ]; then
    ...
    if [ "${DRY_RUN:-0}" -eq 1 ]; then
    ...
    else
      {
        ...
        echo "GH_REPO=server-shop-sale-next"
        ...
        echo "STOREFRONT_CONTAINER_NAME=server-shop-sale-next-web"
        echo "COMPOSE_PROJECT_NAME=server-shop-sale-next"
  ...
  local NEW_ALIAS_CANARY="sale-next-canary"
